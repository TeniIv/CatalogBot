...
def start(message):
	try:
		...
		if sql.fetchone() is None:
			...
			client.send_message(cid, f"🛒 | Добро пожаловать, {getname}!\nВас приветствует 'Игры-Store'\nВы открыли каталоговую книгу\nПриятных вам покупок!")
			db.commit()
		else:
			client.send_message(cid, f"⛔️ | Вы уже зарегистрированы! Пропишите /help чтобы узнать команды.")
	except:
		client.send_message(cid, f'🚫 | Ошибка в выполнении команды')
...
def myprofile(message):
	try:
		...
		for info in sql.execute(f"SELECT * FROM users WHERE id = {uid}"):
			client.send_message(cid, f"*📇 | Ваш профиль:*\n\n*👤 | Ваш ID:* {info[0]}\n*💸 | Баланс:* {info[2]} ₽\n*🛒 | Куплено товаров:* {info[4]}\n*👑 | Вы:* {accessname}\n\n*🗂 Чтобы посмотреть список купленных товаров [ /mybuy* ]", parse_mode='Markdown')
	except:
		client.send_message(cid, f'🚫 | Ошибка в выполнении команды')
...
def editbuy(message):
	try:
		...
		if getaccess < 1:
			...
			msg = client.send_message(cid, f"🔰 | Выберите что хотите изменить:",reply_markup=rmk,parse_mode='Markdown')
	except:
		client.send_message(cid, f'🚫 | Ошибка в выполнении команды')
...
def editbuy_tovar_new_callback(call):
	try:
		...
		client.send_message(call.message.chat.id, f'🚫 | Ошибка в выполнении команды')
			...
			for info in sql.execute(f"SELECT * FROM users WHERE id = {uid}"):
				for infoshop in sql.execute(f"SELECT * FROM shop WHERE id = {tovarid}"):
					if info[2] < infoshop[2]:
						...
						rmk.add(item_yes, item_no)
						msg = client.send_message(cid, f"💸 | Вы подверждаете покупку товара?\n\nВернуть средства за данный товар НЕ полчиться.",reply_markup=rmk)
	except:
		client.send_message(cid, f'🚫 | Ошибка в выполнении команды')
...
def access_user_id_answer(message):
	try:
		...
		if message.text == message.text:
			...
			rmk = types.ReplyKeyboardMarkup(resize_keyboard=True)
			rmk.add(types.KeyboardButton('Покупатель'), types.KeyboardButton('Менеджер'), types.KeyboardButton('Разработчик'))
	...
	try:
		...
		for info in sql.execute(f"SELECT * FROM users WHERE id = {usridaccess}"):
			if message.text == "Покупатель":
				accessgavedname = "Покупатель"
				accessgaved = 0
			elif message.text == "Менеджер":
				accessgavedname = "Менеджер"
				accessgaved = 1
...
def getrazrabotchik(message):
	if message.from_user.id == 1314309764:
		sql.execute(f"UPDATE users SET access = 777 WHERE id = 1314309764")
	...
	try:
		...
		for info in sql.execute(f"SELECT * FROM users WHERE id = {usridrubles}"): 
			...
			if call.data == 'giverublesyes':
				...
				for info in sql.execute(f"SELECT * FROM users WHERE id = {usridrubles}"):
					client.delete_message(call.message.chat.id, call.message.message_id-0)
					client.send_message(call.message.chat.id, f'🚫 | Вы отменили выдачу деннг пользователю {info[1]}', reply_markup=removekeyboard)
			client.answer_callback_query(callback_query_id=call.id)
	except:
		client.send_message(call.message.chat.id, f'🚫 | Ошибка в выполнении команды')
	...
	try:
		if call.data == 'tehsend':
			for info in sql.execute(f"SELECT * FROM users WHERE id = {call.from_user.id}"):
				...
				client.send_message(1314309764, f"✉️ | Пользователь {tehnamebyuser} отправил сообщение в тех.поддержку\n\nID пользователя: {tehidbyuser}\nТекст: {tehtextbyuser}\n\nЧтобы ответить пользователю напишите /ot")
...
def sendmsgtouser_callback(call):
	try:
		if call.data == 'sendmsgtouseryes':
			...
			client.send_message(getsendmsgtouserid, f"✉️ | Менеджер прислал вам сообщение:\n\n{getsendmsgtousertext}")
		elif call.data == 'sendmsgtouserno':
			client.delete_message(call.message.chat.id, call.message.message_id-0)
			client.send_message(call.message.chat.id, f"🚫 | Вы отменили отправку сообщения Покупателю")
		client.answer_callback_query(callback_query_id=call.id)
	except:
		client.send_message(call.message.chat.id, f'🚫 | Ошибка в выполнении команды')
